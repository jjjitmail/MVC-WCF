...
{
    public class VerlengenBundel : ScrapingBase
    {
        public VerlengenBundel() : base() { }
                ...
                if (certificateCollection.Count > 0)
                {
                    ...
                    if (String.IsNullOrEmpty(strErrorMessage))
                    {
                        ...
                        else
                        {
                            ...
                            // popup
                            //https://b2b.telfort.nl/custcare/custsvc/basebusiness/changeProductAction.do?method=initSubsInfo4MainProduct&subsId=3101005725766
                            ...
                            using (UnitOfWork session1 = new UnitOfWork())
                            {
                                ...
                                _stList.ToList().ForEach(item =>
                                {
                                    Thread.Sleep(1000);
                                    Random = Ult.CreateRandomString(16);
                                    dictPostData.Clear();
                                    dictPostData.Add("method", "checkProduct");
                                    dictPostData.Add("prodid", item.Value);
                                    dictPostData.Add("subsID", strSubscriberId);
                                    dictPostData.Add("random", "0." + Random);

                                    strResponseData = ScrapeHelper.DoPostPage(
                                        new Uri(string.Format("https://b2b.telfort.nl/custcare/custsvc/basebusiness/contractRetentionAction.do?method=checkProduct&prodid={0}&subsID={1}&random=0.{2}", item.Value, strSubscriberId, Random)),
                                        cookies,
                                        strResponseData,
                                        dictPostData,
                                        "",
                                    ...
                                    dictPostData.Clear();
                                    dictPostData.Add("method", "getContractPeriodByProductId");
                                    dictPostData.Add("productId", item.Value);

                                    strResponseData = ScrapeHelper.DoPostPage(
                                        new Uri(string.Format("https://b2b.telfort.nl/custcare/custsvc/basebusiness/changeProductAction.do?method=getContractPeriodByProductId&productId={0}", item.Value)),
                                        cookies,
                                        strResponseData,
                                        dictPostData,
                                        "",
                                    ...
                                    ////////
                                    strResponseData = strResponseData.RemoveSpaceAndBreak();
                                    Domain.InitContractPeriodeVerlengen(strResponseData).ForEach(m =>
                                        {
                                            //
                                            dictPostData.Clear();
                                            ...
                                            dictPostData.Add("subsId", strSubscriberId);
                                            dictPostData.Add("key", "product_ID");
                                            dictPostData.Add("value", item.Value);

                                            strResponseData = ScrapeHelper.DoPostPage(
                                                new Uri(string.Format("https://b2b.telfort.nl/custcare/custsvc/basebusiness/contractRetentionAction.do?method=updateSession&subsId={0}&key=product_ID&value={1}", strSubscriberId, item.Value)),
                                                cookies,
                                                strResponseData,
                                                dictPostData,
                                                "",
                                            ...
                                            dictPostData.Add("subsId", strSubscriberId);
                                            dictPostData.Add("productId", item.Value);
                                            dictPostData.Add("contractPeriod", m.Value);

                                            strResponseData = ScrapeHelper.DoPostPage(
                                                new Uri(string.Format("https://b2b.telfort.nl/custcare/custsvc/basebusiness/changeProductAction.do?method=initSubsInfo4VasProduct&subsId={0}&productId={1}&contractPeriod={2}", strSubscriberId, item.Value, m.Value)),
                                                cookies,
                                                strResponseData,
                                                dictPostData,
                                                "",
                                            ...
                                            //hier begin scraping bundel data
                                            ...
                                            int Count = Domain.CountWords(strResponseData, "<tr basenode=\"1\"");
                                            ...
                                            while (strResponseData.Contains("<tr basenode=\"1\""))
                                            {
                                                ...
                                                if (HtmlNode.Contains("radio"))
                                                {
                                                    ...
                                                    kortingsperiodeValue = kortingsperiodeValue == "" ? "0" : kortingsperiodeValue;
                                                    ...
                                                    _Lookup_AboBundel.KeyCombo = string.Format("Verlengen;{0};{1};{2};{3}", item.Name, ProductGroupName, nm, m.Value);
                                                    _Lookup_AboBundel.BundelId = productID + Ult.CreateRandomCharacter(5) + m.Value;
                                                    _Lookup_AboBundel.IsNieuw = false;
                                                    _Lookup_AboBundel.ProductType = (int)Lookup_AboBundel.ProductKeuze.NORMAL;
